...
class Keyboard:
    ...
    def __str__(self):
        ...
        keyboard = ''
        for i in range(10):
            keyboard += color_word(list(self.colors.values())[i], letters[i])
            keyboard += ' '
...
def fast_sort(lst):
    ...
    for i, num in enumerate(lst):
        if num < lst[pivot]:
            ...
            middle.append(num)
    return fast_sort(fewer) + middle + fast_sort(greater)
...
def get_feedback_colors(secret_word, guessed_word):
    ...
    for i in range(NUM_LETTERS):
        if guessed_word[i] == secret_word[i]:
            ...
            secret_word = secret_word[0:i] + '1' + secret_word[i+1:5]
    for i in range(NUM_LETTERS):
        if secret_word[i] == '1':
            pass
            ...
            for n in range(5):
                if guessed_word[i] == secret_word[n]:
                    secret_word = secret_word[0:n] + '0' + secret_word[n+1:5]
                    break
        else:
...
def get_feedback(remaining_secret_words, guessed_word):
    ...
    for key, value in families.items():
        family = WordFamily(key, value)
        families_lst.append(family)
    sorted_families_lst = fast_sort(families_lst)
    return(sorted_families_lst[0].feedback_colors, sorted_families_lst[0].words)
